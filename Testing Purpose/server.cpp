...
int main() {
    ...
    udp::endpoint local_endpoint = udp::endpoint(ip::address::from_string("0.0.0.0"), 1234);
    //socket creation 
    ...
    socket_.send_to(buffer(connection_add[1] + " " + std::to_string(connection_port[1])), client2);
    socket_.send_to(buffer(connection_add[0] + " " + std::to_string(connection_port[0])), client1);
}

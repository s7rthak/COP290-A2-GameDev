...
SDL_Rect score_src, score_dest;
SDL_Rect lives_src, lives_dest;
    ...
    for (int i = 0; i < 31; i++) {
        for (int j = 0; j < 31; j++) {
            if (map[i][j] == WALL && map[i+1][j] == WALL && map[i][j+1] == WALL && map[i+1][j+1] == WALL) {
                ...
                TextureManager::Draw(wall, src, dest, 0.0f, false);
            }
            if (map[i][j] == FOOD) {
                dest.x = j * TILE_WIDTH;
                dest.y = i * TILE_HEIGHT;
                TextureManager::Draw(food, src, dest, 0.0f, false);
            }
        }
    }
    ...
    for (int i = 0; i < 32; i++) {
        for (int j = 0; j < 32; j++) {
            if (map[i][j] == PACMAN) {
                ...
                {
                    case 0:
                        TextureManager::Draw(pacman_fo, src, dest, angle, flip);
                        break;
                    
                    case 1:
                        TextureManager::Draw(pacman_ho, src, dest, angle, flip);
                        break;

                    case 2:
                        TextureManager::Draw(pacman_c, src, dest, angle, flip);
                        break;
